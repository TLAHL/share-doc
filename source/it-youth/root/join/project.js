(function() {
    var e = Project = function(map) {
        this.CLASS_NAME = 'project';
        // 非法输入标志
        this.regFlagObj = {};
        this.datas = {};
        // 必填项
        this.requiredContent = ['name', 'contact', 'phone', 'email', 'school', 'major', 'email', 'city', 'overview', 'members'];
        // 必填项标志
    ...
    e.prototype.regVal = function(type) {
        ...
        if (flag === false) {
            ...
            project.regFlagObj[type] = false;
        } else {
            $(".star." + type).removeClass("red");
            project.regFlagObj[type] = true;
        }

        if (type === "memberschool" || type === "membername" || type === "membermajor") {
            project._regValMembers.call(this, type, reg);
        }
    };
    // 验证团队成员
    ...
    e.prototype.submit = function() {
        ...
        $("#submit").attr("onclick", "");
        // 发送请求，保存个人报名表
        this._saveProjectTable();
    };

    e.prototype._saveProjectTable = function() {
        $.ajax({
            ...
            success: function(data) {
                if (data.result === "success") {
                    project._showDetail();
                    $("#submit").attr("onclick", "project.submit()");
                } else {
                    // 有未完成项，使用toast提示
    ...
    e.prototype._showDetail = function() {
        ...
        jQuery.get("/youth/root/join/project-detail.html", function(html) {
            var template = Handlebars.compile(html);
            $('.detail').append(template());
            for (var key in project.datas) {
                if (key === "id") {
                    $('#detail').data("id", project.datas[key]);
                    $(".detail #" + key).text(project.datas[key]);
                } else if (key === "education" || key === "sex") {
                    var temp = project.datas[key];
                    var value = datasConfig[key][temp];
                    project.datas[key] = value;
                    $(".detail #" + key).eq(0).text(project.datas[key]);
                } else if (key === "sendTime") {
                    var temp = project.datas[key];
                    project.datas[key] = "报名日期：" + temp;
                    $(".detail #" + key).text(project.datas[key]);
                } else if (key === "members") {
                    project._setMembers(project.datas);
                } else if (key === "overview") {
                    var dataTemp = project.datas.overview.replace(/\n/g, "<br>");
    ...
    // 获取项目信息
    e.prototype._getProjectInfo = function() {
        var members = [];
        var member = {};
        ...
        for (var key in this.datas) {
            ...
            } else if (key === 'sendTime' || key === "education" || key === 'members') {
                continue;
            } else {
